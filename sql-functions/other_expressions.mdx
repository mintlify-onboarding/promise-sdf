...
`array(values)` → _list_ Creates a list of elements
...
`random()` → _double_ Creates a pseudo-random number between 0 and 1
...
`sha224()` → _bytearray_ Computes the SHA224 hash of string.
...
`sha256()` → _bytearray_ Computes the SHA256 hash of string
...
`sha384()` → _bytearray_ Computes the SHA384 hash of string
...
`sha512()` → _bytearray_ Computes the SHA512 hash of string
...
`to_hex()` → _varchar_ Computes the hexadecimal representation of an integer
