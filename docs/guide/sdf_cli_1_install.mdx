...
## Prerequisites
SDF is currently supported on the following platforms. The executable is small (<50mb), but heavily parallelized. As such, SDF's resource consumption is dependent entirely on your data processing requirements.
...
For a complete list of releases, please see the official [github releases page](https://github.com/promise-labs/sdf-cli/releases).
...
If you've already installed SDF, you can update to the latest version by running the command below. By default, this will update to the latest version of the cli. But, you can also pass `--version` and `--install-location` parameters.
```_run shell
$sdf update
```
